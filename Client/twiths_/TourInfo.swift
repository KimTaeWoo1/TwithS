...
class TourInfo: UITableViewController {

    var This_Tour:Tour = Empty_Tour
    
    @IBOutlet var bar1: UINavigationItem!
    @IBOutlet var TourInfoTable: UITableView!
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row < 10 {
            ...
            cell.MapViewImg.image = UIImage(named: This_Tour.MapImage)
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        /*
        if segue.identifier == "return_from_tourinfo" {
            let dest = segue.destination as! Tour_Landmarks
            dest.ID = This_Tour.TourID
